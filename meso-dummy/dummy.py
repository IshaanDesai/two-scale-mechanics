...
for name in write_data_names:
    write_data[name] = np.zeros(
        (num_vertices, participant.get_data_dimensions(mesh_name, name))
    )
...
for x in range(num_vertices):
    ...
    write_data["strains1to3"][x, :] = np.random.uniform(
        0, 1e-4, 3
    )  # Random strain values
    write_data["strains4to6"][x, :] = np.random.uniform(
        0, 1e-6, 3
    )  # Random strain values
