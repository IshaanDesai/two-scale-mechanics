...
class Mesh:
    ...
    def load(self, config: Config):
        ...
        if not self.bc_nm_use_tag:
            ...
            self.facet_markers = mesh.meshtags(
                mesh_data.mesh, 2, facet_indices, facet_values
            )
        else:
        ...
        if self.bc_dc_use_tag:
            ...
            for i in range(3):
                facets = mesh.locate_entities_boundary(
                    self.domain, fdim, self.bc_dc_locator
                )
                dofs = fem.locate_dofs_topological(V.sub(i), fdim, facets)
                bcs.append(fem.dirichletbc(self.bc_dc_value, dofs, V.sub(i)))
    ...
    def get_bc_neumann(self, v):
        ...
        """
        return ufl.inner(self.bc_nm_value, v[self.bc_nm_dim]) * self.ds(
            Locators.NEUMANN_TAG
        )
