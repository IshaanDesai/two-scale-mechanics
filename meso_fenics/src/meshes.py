...
class Bar(Mesh):
    def __init__(self):
        super().__init__()
        mesh_data = io.gmsh.read_from_msh('res/bar3d.msh', MPI.COMM_WORLD, gdim=3)
        self.domain = mesh_data.mesh
        self.cell_markers = mesh_data.cell_tags
        self.facet_markers = mesh_data.facet_tags
        self.dimensions = Dimensions([0.0, 0.0, 0.0], [1.0, 0.25, 0.25], [5, 2, 2])
        self.normalize_domain()
        self.ds = ufl.Measure('ds', domain=self.domain, subdomain_data=self.facet_markers)

        self.bc_dc_use_tag = False
        self.bc_dc_locator: Optional[Callable] = Locators.plane_yz_low
        self.bc_dc_value = fem.Constant(self.domain, 0.)
        self.bc_nm_use_tag = True
        self.bc_nm_locator: Optional[Callable] = None
        self.bc_nm_value = fem.Constant(self.domain, -0.01)
        self.bc_nm_dim = 1

    def load(self, config: Config): pass


class Notch(Mesh):
    def __init__(self):
        super().__init__()
        mesh_data = io.gmsh.read_from_msh('res/notch.msh', MPI.COMM_WORLD, gdim=3)
        # setup facet markers
        facet_markers = mesh_data.facet_tags
        facet_indices = facet_markers.indices.copy()
        facet_values = facet_markers.values.copy()
        com = mesh.compute_midpoints(mesh_data.mesh, 2, facet_markers.indices)
        com_y = com[:, 1]
        facet_values[np.isclose(com_y, 55, atol=1e-8)] = Locators.NEUMANN_TAG
        ...
        self.dimensions = Dimensions([0.0, 0.0, 0.0], [30.0, 110.0, 5.0], [0, 0, 0])
        self.normalize_domain()
        self.ds = ufl.Measure('ds', domain=self.domain, subdomain_data=self.facet_markers)

        self.bc_dc_use_tag = False
        self.bc_dc_locator: Optional[Callable] = Locators.plane_xz_low
        self.bc_dc_value = fem.Constant(self.domain, 0.)
        self.bc_nm_use_tag = True
        self.bc_nm_locator: Optional[Callable] = None
        self.bc_nm_value = fem.Constant(self.domain, 0.01)
