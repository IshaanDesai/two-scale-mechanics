...
#
# Job name:
#SBATCH -J scale-micro-bar-sphere512-node
#
...
edit::fans_input_switch ./input.json ${NUM_MM_WORKERS} "sphere512.h5"
gen_host_files ${NUM_MESO_NODES} ${NUM_MM_NODES}

echo "Starting Meso Simulations"
touch "${OUT_DIR}/meso.log"
cd "${MESO_PATH}" || return
(I_MPI_PIN=1 I_MPI_PIN_CELL=core I_MPI_PIN_PROCESSOR_LIST=40,41,42,43,44,45,46,47 mpiexec -hostfile hosts.meso -n 8 -ppn 8 python macro.py "${JOB_DIR}/config-meso.json" &> "${OUT_DIR}/meso.log")&
