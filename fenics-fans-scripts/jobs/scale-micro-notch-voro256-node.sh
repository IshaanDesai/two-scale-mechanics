...
#
# Job name:
#SBATCH -J scale-micro-notch-voro256-node
#
...
MICRO_PATH="${TSM_PATH}/micro-fans-notch-voro"
...
cp $MICRO_PATH/voro256.h5 ./

edit::meso_input ./config-meso.json "${OUT_DIR}/meso-geom" "${OUT_DIR}/meso-state" "${JOB_DIR}/precice-config.xml"
edit::precice_input ./precice-config.xml "${OUT_DIR}/precice.log" "${OUT_DIR}/precice-profiling" "${OUT_DIR}/precice-exports" "${JOB_DIR}" "${NUM_NODES}"
edit::mm_input ./micro-manager-config.json "${JOB_DIR}/precice-config.xml" ${NUM_MM_RANKS} ${NUM_MM_WORKERS}
edit::fans_input_switch ./input.json ${NUM_MM_WORKERS} "voro256.h5"
gen_host_files ${NUM_MESO_NODES} ${NUM_MM_NODES}

echo "Starting Meso Simulations"
touch "${OUT_DIR}/meso.log"
cd "${MESO_PATH}" || return
(I_MPI_PIN=1 I_MPI_PIN_CELL=core I_MPI_PIN_PROCESSOR_LIST=40,41,42,43,44,45,46,47 mpiexec -hostfile hosts.meso -n 8 python macro.py "${JOB_DIR}/config-meso.json" &> "${OUT_DIR}/meso.log")&

echo "Starting Micro Simulations"
touch "${OUT_DIR}/micro.log"
cd "${JOB_DIR}" || return
I_MPI_PIN=1 I_MPI_PIN_CELL=core I_MPI_PIN_DOMAIN=${NUM_MM_WORKERS} I_MPI_PIN_ORDER=compact mpiexec -hostfile hosts.micro -n ${NUM_MM_RANKS} -ppn ${NUM_MM_PPN} micro-manager-precice "${JOB_DIR}/micro-manager-config.json" &> "${OUT_DIR}/micro.log"
