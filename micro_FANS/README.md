...
> Note: Discuss with the DAE team before publishing. This repository is currently private.
...
The compilation will symlink the generated PyFANS binary into the `test/` directory for convenience.
...
- set the path to the input file and output file (this is not used but required by FANS) in the `input.json` (this name is fixed) file that must be located at the same directory as the pybind11 PyFANS library.
